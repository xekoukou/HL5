...
return : ∀{α} → ∀{@(tactic ihs) HA : HSet {α}} → ⟦( HA → ○ HA )⟧′
return {HA = HA} {w} ha = I.return {IA = λ w → HA {w}} w ha

_>>=_ : ∀{α β} → {@(tactic ihs) HA : HSet {α}} {HB : HSet {β}}
        → ⟦( ○ HA → (HA → ○ HB) → ○ HB )⟧′
...
□-refl : ∀{α} → ∀{HA : HSet {α}} → {w : World} → □ HA → HA
□-refl {w = w} □a = □a w

K : ∀{α} → ∀{HA HB : HSet {α}} → {w : World} → □ (λ {_} → HA → HB) → □ HA → HB
K {w = w} □f □a = □f w (□a w)
